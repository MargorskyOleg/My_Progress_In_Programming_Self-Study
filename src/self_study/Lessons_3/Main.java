...
public class Main {
    ...
    private static final char[][] MATRIX_ENCRYPTED = {   //                  x               x     координаты
    ...
    public static void main(String[] args) {
        ...
        PlayFeirDecryption decoder = new PlayFeirDecryption(MATRIX_ENCRYPTED);

        decoder.splitTheStringIntoPairsOfSmvolov(ENCRYPTED_MESSAGE);
...
//        byte byteSize = Byte.MAX_VALUE;
//        System.out.println(byteSize);//127
//
//        short shortSize = Short.MAX_VALUE;
//        System.out.println(shortSize);//32767
//
//        int intSize = Integer.MAX_VALUE;
//        System.out.println(intSize);//2147483647
//
//        long longSize = Long.MAX_VALUE;
//        System.out.println(longSize);//9223372036854775807
//
//        float floatSize = Float.MAX_VALUE;
//        System.out.println(floatSize);//3.4028235E38
//
//        double doubleSize = Double.MAX_VALUE;
//        System.out.println(doubleSize);//1.7976931348623157E308
//
//        char charSize = Character.MAX_HIGH_SURROGATE;
...
//actually we don't need this varibale any more as we declared constant ENCRYPTED_MESSAGE /на самом деле эта вариабельность нам больше не нужна, так как мы объявили константу ENCRYPTED_MESSAGE
...
//String decodedMessage = decoded(encryptedMessage); //Why variable was named messageS ('s' on the end is mean plural form), it should be just message or even better decodedMessage /Почему переменная была названа messageS ('s' в конце означает форму множественного числа), это должно быть просто message или даже лучше decodedMessage

//System.out.println("\n\n--- Result of decoded method ---"); /Результат декодированного метода
//System.out.println(decodedMessage);
            ...
            //actually a is not so imformative name for the argument variable /на самом деле это не так уж информативное имя для аргумента переменной
            //better to use encodedString or encryptedString or just originalString (as method named decode it should be clear what is original string) /лучше использовать encodedString или encryptedString или просто originalString (в качестве метода с именем decode должно быть понятно, что является исходной строкой)
//    private static String decoded(String encryptedString) {//метод раскодировки
//
//        int i = 0;
//        int j = 1;
//        int b = 2;
//        //int num = a.length(); //why do you need variable num? You can use just a.length everywhere /зачем вам переменная num? Вы можете использовать только длину. Везде
//        char letter_1;
//        char letter_2;
//        char n = '\n';
...
//            j = j + b;
//
//            //let's check pair of symbols from encrypted string :) /давайте проверим пару символов из зашифрованной строки
//            //System.out.println("Pair of symbols from encrypted string: " + letter_1 + letter_2); /Пара символов из зашифрованной строки:
//            //XF PV ... where are first symbols? Upssss. /где первые символы? Upssss.
//
//            //Let's try to do it differently /Давайте попробуем сделать это по-другому
//
